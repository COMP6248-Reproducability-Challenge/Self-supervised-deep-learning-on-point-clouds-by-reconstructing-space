...
checkpoint = torch.load(f'pretrained_40.state')
...
dataset = H5Dataset('train_files.txt')

ptclouds, labels = dataset.models, dataset.labels

labelslist = list(map(lambda e: e.item(), labels))

percentages = [.01, .02, .1, .2, .5, 1.]

labelsset = set(range(labels.shape[0]))

testloader  = DataLoader(H5Dataset('test_files.txt'), batch_size=32, shuffle=True)

for percentage in percentages:
    if percentage == 1.:
        cloudindices = range(labels.shape[0])
    else:
        cloudindices = [list(np.random.permutation(labelslist)).index(i) for i in range(40)]

        amount = int(percentage * ptclouds.shape[0])

        print('Sampling point clouds...')
        while len(cloudindices) < amount:
            current = np.random.choice(list(labelsset - set(cloudindices)))

            cloudindices.append(current)
        print('Sampling done.')

    trainloader = DataLoader(
            list(zip(zip(ptclouds[cloudindices], torch.zeros(ptclouds[cloudindices].shape[0], 16, 1).to(device)), labels[cloudindices])),
            batch_size=32,
            shuffle=True
    )
    ...
    embeddings, embeddings_lbls = np.empty((count, 1024, 1)), np.empty((count))

    acc = 0

    for tuples, lbls in tqdm(trainloader):
        preds = get_embeddings(model, layer, tuples, lbls.shape[0])
        
        embeddings[acc:acc+preds.shape[0]]      = preds
        embeddings_lbls[acc:acc+preds.shape[0]] = lbls.cpu().numpy()

        acc += preds.shape[0]

    print('Fitting SVM...')
    svc.fit(embeddings.squeeze(2), embeddings_lbls)
    print('Done.')
    ...
    for tuples, lbls in tqdm(testloader):
        preds = get_embeddings(model, layer, tuples, lbls.shape[0])
    ...
    accuracy = svc.score(embeddings.squeeze(2), embeddings_lbls) * 100

    print(f'Accuracy at {int(percentage*100)}%: {accuracy:.4f}%')

    with open('perc2048.txt', 'a') as f:
        f.write(f'{int(percentage*100)}, {accuracy:.4f}\n')
